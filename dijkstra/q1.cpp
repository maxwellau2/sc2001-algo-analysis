...
struct DijkstraReturn {
    vector<int> distances;  // Shortest distances from the start node
                            // (equivalent to 'd' in lecture notes)
    vector<int> pi;  // Predecessor array to track paths (equivalent to 'pi' in
                     // lecture notes)
...
// Function to extract the node with the minimum distance from the priority
// queue
int extractCheapest(vector<pair<int, int>>& priorityQueue,
                    uint64_t& key_comps) {
    int minIndex = 0;
    ...
    priorityQueue.erase(priorityQueue.begin() +
                        minIndex);  // Remove the extracted node
    return minNode;
}

// Modified Dijkstra's algorithm to track key comparisons and predecessor array
// using a vector-based priority queue

DijkstraReturn dijkstra(const vector<vector<int>>& graph, int startNode) {
    int n = graph.size();
    vector<int> distances(
        n, INT_MAX);  // 'd' array in lecture notes: shortest path estimates
    vector<int> pi(
        n, -1);  // 'pi' array in lecture notes: predecessors of each vertex
    vector<bool> visited(
        n, false);  // 'S' in lecture notes: indicates if a node is processed
    vector<pair<int, int>> priorityQueue;  // Vector-based priority queue to
                                           // hold (node, distance) pairs
    uint64_t key_comps = 0;                // Counter for key comparisons

    distances[startNode] =
        0;  // Distance from the source node to itself is always 0
    ...
    while (!priorityQueue.empty()) {
        int u = extractCheapest(
            priorityQueue,
            key_comps);     // Get the node with the smallest distance
        visited[u] = true;  // Mark the node as processed (add to set 'S')

        // Update distances for neighboring nodes
        for (int v = 0; v < n; ++v) {
            if (graph[u][v] != 0 &&
                !visited[v]) {  // Check for connected unvisited nodes
                key_comps++;
                int newDistance = distances[u] + graph[u][v];
                if (newDistance < distances[v]) {  // If a shorter path is
                                                   // found, update the distance
                    key_comps++;
    ...
    return {distances, pi,
            key_comps};  // Return distances, predecessors, and key comparisons
}
...
//         auto duration = chrono::duration_cast<chrono::nanoseconds>(end -
//         start).count();

//         lock_guard<mutex> lock(fileMutex); // Lock the mutex for thread-safe
//         file writing file << num_nodes << "," << result.key_comps << "," <<
//         duration << "\n";
//     }
    ...
    for (int num_nodes = 500; num_nodes <= 10000; num_nodes += 100) {
        ...
        for (int iter = 0; iter < iterations; ++iter) {
            vector<vector<int>> graph(
                num_nodes,
                vector<int>(num_nodes, 0));  // Adjacency matrix representation
        ...
        lock_guard<mutex> lock(
            fileMutex);  // Lock the mutex for thread-safe file writing
        file.open("graph_size_results.csv", std::ios::app);
...
// Function to test graphs of specific sizes with increasing density and record
// results into a CSV file void testGraphsWithSpecificSizesAndDensity() {
...
//         int max_possible_edges = num_nodes * (num_nodes - 1);  // Maximum
...
//             for (int i = 0; i < num_nodes - 1; ++i) {
//                 int weight = rand() % 20 + 1;  // Random weight between 1 and
//                 20 graph[i][i + 1] = weight;
//                 // graph[i + 1][i] = weight;      // Undirected graph
//             }

//             // Add additional random edges to achieve the desired edge count
//             int added_edges = num_nodes - 1; // Already added by the spanning
//             tree while (added_edges < current_edge_count) {
...
//             lock_guard<mutex> lock(fileMutex); // Lock the mutex for
//             thread-safe file writing file << num_nodes << "," << (interval *
//             10) << "," << result.key_comps << "," << duration << "\n";
//         }
//     }

//     file.close();
//     cout << "Graph density test results saved to
//     graph_density_results.csv\n";
// }
// Function to test graphs of specific sizes with increasing density and record
// results into a CSV file (adjacency matrix implementation)
void testGraphsWithSpecificSizesAndDensity() {
    ofstream file;
    file.open("graph_density_results.csv", std::ios::app);
    file << "Number of Nodes,Density (%),Average Key Comparisons,Average CPU "
            "Time (us)\n";
    file.close();
    ...
    for (int num_nodes : specific_sizes) {
        int max_possible_edges =
            num_nodes *
            (num_nodes - 1);  // Maximum number of edges for full connectivity
        ...
        for (int interval = 1; interval <= 20; ++interval) {
            ...
            for (int iter = 0; iter < iterations; ++iter) {
                int current_edge_count = (interval * max_possible_edges) / 20;
                vector<vector<int>> graph(
                    num_nodes, vector<int>(num_nodes, 0));  // Adjacency matrix
                ...
                // Add additional random edges to achieve the desired edge count
                int added_edges =
                    num_nodes - 1;  // Already added by the spanning tree
            ...
            lock_guard<mutex> lock(
                fileMutex);  // Lock the mutex for thread-safe file writing
            file.open("graph_density_results.csv", std::ios::app);
            file << num_nodes << "," << (interval * 5) << ","
                 << average_key_comps << "," << average_duration << "\n";
            file.close();
        }
    }
